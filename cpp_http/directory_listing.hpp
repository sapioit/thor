...
static std::string make_link(const std::string &uri, const boost::filesystem::path &p) {
    std::string link = uri;
    if (uri.back() != '/')
        link += '/';
...
static std::string make_file_name(const boost::filesystem::path &p) {
    if (boost::filesystem::is_directory(p)) {
...
static boost::filesystem::path relative_to(boost::filesystem::path from, boost::filesystem::path to) {
...
static std::string parent_directory_anchor(std::string uri, std::string root_path) {
